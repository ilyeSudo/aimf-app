...
class CarouselImages extends React.PureComponent {
    ...
    numericPagination = (currentIndex: number, lastIndex: number) => {
        return (
            <View style={{alignSelf: 'stretch', alignItems: 'flex-end', paddingRight: 15}}>
    ...
    showImageModal = () => {
        return (
            <Modal visible={this.state.showImageModel}
                   transparent={true}
                   onRequestClose={() => this.setState({showImageModel: false})}
            >
                <ImageViewer
                    imageUrls={this.props?.images.map(
                        m => {
                            return {
                                url: getUrlImage(m),
                                props: {},
        ...
        return (
            <View style={{paddingTop: 35}}>
                <View style={{alignItems: 'center'}}>
                    ...
                    />
                    {this.numericPagination(this.state?.currentIndex, this.state?.totalIndex)}
                    {this.showImageModal()}
                </View>
            </View>
        );
